...
for (let i = 0; i < food_type.length; i++) {
    let k = getRandomIntInclusive(0, 3)
            ...
            if (names.hasOwnProperty(name) === false) {
                if (checkIfAttributesAreNull(name, weight, price, image_url) == true) {
                    ...
                    for (let i = 0; i < rep_count; i++) {
                        ...
                        let rand_percent = getRandomIntInclusive(1, 12) / 100

                        let rep_price = (getRandomIntInclusive(1, 2) === 1) ? price + (price * rand_percent) : price - (price * rand_percent)
                        const store_obj = {
                            "name": cur_store["name"],
                        ...
                        //Authorization : bazaara-integration-test
                        ...
                        let prod_obj = {
                            "name": String(name),
                            "upc_code": String(rand_upc),
                            "price": Number(rep_price.toFixed(2)),
                            "store": store_obj,
                            "image_url": String(image_url),
                            "weight": String(weight)
                        }

                        dataset.push(prod_obj)
                        console.log("prod_obj", prod_obj)
                    }
                }
            }
